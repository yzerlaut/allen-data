...
manifest_path = os.path.join(data_directory, 
                             "manifest.json")
...
for Sessions, Key in zip([PV_sessions, SST_sessions, VIP_sessions],
    ...
    for iSession in range(len(Sessions)):
        ...
        fig.savefig(os.path.join('figures', 
                    Key+'-Optotagging-%i.png' % (iSession+1)))
        plt.close(fig)
...
# considering all units in V1
#np.save(os.path.join('data', 'Optotagging-Results-VISp.npy'), Optotagging)
# considering all units in the visual cortex
np.save(os.path.join('data', 'Optotagging-Results.npy'), Optotagging)

# %%
Optotagging = np.load(os.path.join('data',
